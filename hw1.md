...
本阶段使用 `assets/bg1a.png`作为固定背景，使游戏画面更符合空战主题。

**背景素材要求**

1. 背景图片使用 `assets/bg1a.png`。
2. 图片尺寸为 `500 × 900`，能够覆盖 `500 × 800` 的游戏窗口；超出窗口的部分由 Pygame 自动裁剪，无需额外缩放。
...
在 `settings.py` 中统一配置音频路径和音量：开始音乐 `0.14`、游戏音乐 `0.25`、敌机爆炸 `0.25`、玩家坠毁 `0.20`。

...
加入精确的玩家碰撞、开始与结束界面、重新开始和中英文文本，使项目从“功能演示”成为一局流程完整的游戏。
...
1. 分别根据玩家和敌机带透明通道的图片创建 `mask`。
2. 在 `is_mask_collision(sprite1, sprite2)` 中计算两个对象的相对偏移。
3. 使用 `mask.overlap()` 判断两张图片的非透明部分是否重叠。
4. 玩家与任意敌机发生真实图形碰撞时，游戏进入 `GAME_OVER`。

提示：遮罩碰撞的偏移量应为第二个对象坐标减去第一个对象坐标。子弹碰撞仍使用矩形碰撞即可。
